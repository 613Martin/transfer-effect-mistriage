#' RunStudy
#'
#' Inputs a data frame with the data required to run the study.
#' Potentially runs with boostraps, augemented by foreach.
#' Outputs a data frame with medians and IQR.
#' If bootstraps are used, the function will NOT return values to the Results.env
#' or pring sample characteristics tables.
#' If bootstraps are used, the output created will be a row-binded data frame of
#' all the data frames produced from the bootstraps.
#' @param data Data used to run study
#' @param  Logical. If TRUE, the study will run with 1000 bootstraps. Defaults to
#'     FALSE.
RunStudy <- function(data, boot = FALSE) {
