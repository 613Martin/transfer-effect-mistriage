...
# Introduction
Trauma, defined as an injury to living tissue by an extrinsic agent [@RN63], 
results in approximately 3,8 million deaths annually across the globe [@RN1; @RN21]. 
Each year 9% of the world’s deaths are because of trauma, with the leading causes being 
road traffic accidents, suicide and homicide. Predictions indicate the incidence of all 
these causes are likely to increase by 2030 [@RN42].
...
Systems that determine the level of trauma can be based on clinical prediction
models. Models differ in quality and characteristics, but generally perform
well at predicting survival [@RN45]. Many models are developed in a single,
...
subdivided into overtriage, the incorrect classification of a patient with minor
trauma as major trauma, or undertriage, the incorrect classification of a patient
with major trauma as minor trauma. Mistriage can ultimatly lead to decreased patient 
survival, but is also detrimental in terms of patient care and distribution of resources. 
...
substantial knowledge gaps. Therefore, the aim of this study is to assess how transfers 
of clinical prediction models for early (on arrival to hospital) trauma 
care between different care contexts within a single health system affect mistriage rates.
...
clinical prediction models, which were transferred between different care contexts to
...
The trauma care in Sweden is usually clearly defined in major
...
form of system to categorize the level of trauma, usually
...
Currently, the registry includes 55 000 cases, and its use is encouraged in 
both academic research and local quality improvement initiatives [@RN28].

## Participants 
The eligibility criteria were adult patients aged 15 years or above registered in SweTrau 
between 2011 and 2016. The study aimed to assess adult trauma and not
...
part of multiple established clinical prediction models for early trauma care, such
...
hospital. NISS and ISS are calculated by hospital personnel
...
Neither the outcome nor variables were blinded when during analysis, which 
made a structured approach important, to ensure objectivity.

## Study size
All patients matching eligibility criteria were included. Four data sets were
...
High volume centres were those within the top quartile of number of patients received. 
The rest were low volume centres.
...
statistics Sweden, and a second sample of patients from non-metropolitan areas.

### Data set 3: Multi- and single centre data
In this data set, two samples were created. One centre with a large enough 
sample size to develop and validate a model was its own sample. The multicentre 
...
had a large enough sample size to develop and validate a model. However, this 
...
the high- and low volume centre data set as an example. The minimum sample size of development and
...
![](fig1.jpg)  
**Fig. 1: Sample development in the high and low volume centre data set.** Illustrates 
creation of development and validation sample in the high and low volume data set. 
Initial split based on number of patients. Temporal split made using date of traumatic event.
...
high volume centre sample to a low volume centre sample as an example to describe 
...
mortality in each sample. Then the probability cut-off identified in the
...
Ethical review numbers are 2015/426-31 and 2016/461-32. All patients in SweTrau 
had received letters detailing their inclusion in the database, and the possibility 
of their data being used for scientific purposes. It was assumed that the patients 
who consented to the use of their SweTrau data for research want to contribute to 
...
model predictors, was `r round(gcs.na$percentage)`% for GCS and `r round(sbp.na$percentage)`% 
for SBP.
