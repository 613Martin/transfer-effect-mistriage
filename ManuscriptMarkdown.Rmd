...
abstract: |
  ...
  **Background:** Trauma results in approximately 3,8 million deaths annually. Clinical
  ...
  indicated transferring these models between different settings might adversely affect 
  model performance.
...
# Introduction
Trauma results in approximately 3,8 million deaths annually across the globe, 
and road traffic injuries are among the top 10 leading causes of death [@RN1; @RN21]. 
...
Neither outcome nor variable were blinded when conducting data analysis, which 
...
## Statistical methods
### Data sets
The complete SweTrau cohort was split into four sets of data, each representing a 
plausible clinical setting for model transfer. These data sets were further divided 
into samples, this process is outlined below.

### High and low volume centres
Based on number of patients received and registred in SweTrau for each centre, two 
samples were derived from this data set. High volume centres were those with in the 
top quartile of number of patients registred. The rest were low volume centres.
...
sample size (outlined below) to develop and validate a model constituted their own 
sample. The multi-centre sample consisted of the combined data from all centres.
...
validation samples were 140 and 200 respectively. We performed analyses only on
data sets for which all samples included at least the minimum sample size.

![](fig1.jpg)  
**Fig. 1: High and low volume centre data set.** Illustrates creation of development 
and validation sample in the high and low volume data set. Initial split based on
...
trauma and independent variables SBP, RR, and GCS modelled as previously described. 
...
would signifiy an improvement in model performance. The higher 95% UI of 
...
Recognizing these risks and updating models accordingly should lead
